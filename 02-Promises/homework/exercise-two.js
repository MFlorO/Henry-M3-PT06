...
function problemA () {
  ...
  // ???
   ...
   let promisse1 =  promisifiedReadFile("poem-two/stanza-01.txt")
   let promisse2 =  promisifiedReadFile("poem-two/stanza-02.txt")
...
function problemB () {
  ...
  let filesToPromises = filenames.map(file => promisifiedReadFile(file))
  Promise.all(filesToPromises)
    .then(promesas => {
      // Aca toma todas las promesas ya resueltas las recorro y las pinta de azul.
      promesas.forEach(promesa => blue(promesa))
      console.log('done');
    })
